...
@section('content')
  ...
  @push('styles')
    <style>
      ...
      #manage-ngo-table tbody tr {
        transition: background-color 0.2s ease;
      }

      #manage-ngo-table tbody tr:hover {
        background-color: #f7f9fc;
      }

      #manage-ngo-table .badge {
        font-size: 0.72rem;
        font-weight: 600;
      ...
      #manage-ngo-table_wrapper .page-item.active .page-link {
        background-color: #0f0f13 !important;
        border-color: #0f0f13 !important;
        color: #ffffff !important;
      }

      #manage-ngo-table_wrapper .page-item.active .page-link:hover,
      #manage-ngo-table_wrapper .page-item.active .page-link:focus {
  ...
  @endpush
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <div class="card manage-project-head-card">
                <div class="project-top-bar d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                  <div>
                    <h2 class="page-title d-flex align-items-center gap-2"><i class="fa-solid fa-people-group"></i> Manage NGO</h2>
                    <nav aria-label="breadcrumb">
                      <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                        
                        <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('manage-ngo') }}">Manage NGO</a></li>
                      </ol>
                    </nav>
                  </div>
                  @can('add ngo')
                  <a href="{{ route('add-ngo') }}" class="addbtn btn btn-dark">
                ...
                <div class="card project-list-card">
                  ...
                  <div class="card-body">
                    <div class="table-responsive">
                      <table id="manage-ngo-table" class="table table-striped border-top">
                        <thead>
                          <tr>
                            <th>ID</th>
                            <th>Image</th>
        ...
        $(function () {
          ...
          var table = $('#manage-ngo-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('manage-ngo') }}",
            columns: [
          ...
          $(document).on('click', '.item-delete', function () {
            ...
            confirmDelete(function () {
              $.ajax({
                ...
                error: function (xhr) {
                  showToast('error', 'Delete failed: ' + (xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Unknown error'));
                }
              });
            });
          });
          ...
          $(document).on('change', '.toggle-status', function () {
            ...
            $.ajax({
              url: "{{ url('manage-ngo') }}/" + ngoId + "/toggle-status",
              method: 'POST',
              data: { _method: 'PATCH' },
              success: function (response) {
                showToast('success', response.message);
