            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <div class="card manage-project-head-card">
                <div class="project-top-bar d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                  <div>
                    <h2 class="page-title d-flex align-items-center gap-2"><i class="fa-solid fa-eye"></i> View NGO</h2>
                     <nav aria-label="breadcrumb">
                      <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('manage-ngo') }}">Manage NGO</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('view-ngo', $ngo->id) }}">View NGO</a></li>
                      </ol>
                    </nav>
                  </div>
                  ...
                  <div class="card-body">
                    <div class="ngo-profile">
                      <div class="ngo-profile-header">
                        @if($ngo->image)
                        <img src="{{ asset($ngo->image) }}" alt="{{ $ngo->name }}" class="ngo-avatar-lg" />
                        @else
                        <img src="https://placehold.co/92x92/e5e7eb/6b7280?text=NGO" alt="{{ $ngo->name }}" class="ngo-avatar-lg" />
                        @endif
                        <div>
                          <h5 class="ngo-name">{{ $ngo->name }}</h5>
                          <p class="ngo-subtext">Registered NGO Profile</p>
                        </div>
                      </div>

                      <div class="ngo-info-grid">
                        <div class="ngo-info-item">
                          <span class="ngo-info-label">Name</span>
                          <div class="ngo-info-value">{{ $ngo->name }}</div>
                        </div>
                        <div class="ngo-info-item">
                          <span class="ngo-info-label">Contact</span>
                          <div class="ngo-info-value">{{ $ngo->contact }}</div>
                        </div>
                        <div class="ngo-info-item">
                          <span class="ngo-info-label">Email</span>
                          <div class="ngo-info-value">{{ $ngo->email ?: '-' }}</div>
                        </div>
                        <div class="ngo-info-item">
                          <span class="ngo-info-label">City</span>
                          <div class="ngo-info-value">{{ $ngo->city->city_name ?? '-' }}</div>
                        </div>
                        <div class="ngo-info-item">
                          <span class="ngo-info-label">Address</span>
                          <div class="ngo-info-value">{{ $ngo->address ?: '-' }}</div>
                        </div>
                        <div class="ngo-info-item">
                          <span class="ngo-info-label">Created</span>
                          <div class="ngo-info-value">{{ $ngo->created_at->format('d M Y') }}</div>
                        </div>
                        <div class="ngo-info-item">
                          <span class="ngo-info-label">Status</span>
                          <div class="ngo-info-value">{{ $ngo->is_active ? 'Active' : 'Inactive' }}</div>
                        </div>
                      </div>
                    </div>
                  </div>
