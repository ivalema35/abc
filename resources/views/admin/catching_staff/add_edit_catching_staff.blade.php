            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <div class="card manage-project-head-card">
                <div class="project-top-bar d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                  <div>
                    <h2 class="page-title d-flex align-items-center gap-2">
                      <i class="fa-solid {{ $staff->exists ? 'fa-user-pen' : 'fa-user-plus' }}"></i>
                      {{ $staff->exists ? 'Edit Catching Staff' : 'Add Catching Staff' }}
                    </h2>
                    <nav aria-label="breadcrumb">
                      <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">
                          <a href="{{ $staff->exists ? route('manage-catching-staff.edit', $staff->id) : route('manage-catching-staff.create') }}">{{ $staff->exists ? 'Edit Catching Staff' : 'Add Catching Staff' }}</a>
                        </li>
                      </ol>
                    </nav>
                  </div>
                ...
                <div class="card project-list-card">
                  ...
                  <div class="card-body">
                    <form id="catchingStaffForm" method="POST" enctype="multipart/form-data">
                      @csrf
                      <input type="hidden" name="id" id="staffId" value="{{ $staff->id }}">
                      <div class="row g-4">
                        <div class="col-md-6">
                          <label class="form-label" for="staffHospital">Hospital <span class="text-danger">*</span></label>
                          <div class="input-group project-field-group">
                            <select id="staffHospital" name="hospital_id" class="form-select">
                            ...
                            <button class="btn field-picker-btn" type="button" data-bs-toggle="modal" data-bs-target="#hospitalModal">
                        ...
                        <div class="col-md-6">
                          <label class="form-label" for="staffName">Full Name <span class="text-danger">*</span></label>
                          <input type="text" id="staffName" name="name" class="form-control" value="{{ old('name', $staff->name) }}" placeholder="Enter full name" />
                        </div>

                        <div class="col-md-6">
                          <label class="form-label" for="staffCity">City <span class="text-danger">*</span></label>
                          <div class="input-group project-field-group">
                            <select id="staffCity" name="city_id" class="form-select">
                            ...
                            <button class="btn field-picker-btn" type="button" data-bs-toggle="modal" data-bs-target="#cityModal">
                        ...
                        <div class="col-md-6">
                          <label class="form-label" for="staffContact">Contact <span class="text-danger">*</span></label>
                          <input type="tel" id="staffContact" name="contact" class="form-control" value="{{ old('contact', $staff->contact) }}" placeholder="Enter contact number" />
                        </div>

                        <div class="col-md-6">
                          <label class="form-label" for="staffEmail">Email</label>
                          <input type="email" id="staffEmail" name="email" class="form-control" value="{{ old('email', $staff->email) }}" placeholder="Enter email address" />
                        </div>

                        <div class="col-md-12">
                          <label class="form-label" for="staffAddress">Address</label>
                          <textarea id="staffAddress" name="address" class="form-control" rows="3" placeholder="Enter address">{{ old('address', $staff->address) }}</textarea>
                        </div>

                        <div class="col-md-12">
                          <label class="form-label" for="staffImage">Image</label>
                          <input type="file" id="staffImage" name="image" class="form-control" accept="image/*" />
                          <img id="staffImagePreview" class="staff-image-preview" src="" alt="Preview" style="display:none;" />
                        </div>
                      </div>

                      <div class="d-flex flex-wrap gap-2 mt-4 form-action-row">
                        <button type="submit" class="btn bg-black text-white">{{ $staff->exists ? 'Update Catching Staff' : 'Add Catching Staff' }}</button>
                        <a href="{{ route('manage-catching-staff.index') }}" class="btn btn-label-secondary">Cancel</a>
                      </div>
                    </form>
                  </div>
                </div>
...
$(function () {
  ...
  var existingImage = "{{ $staff->exists ? ($staff->image ?? '') : '' }}";
  if (existingImage) {
    $('#staffImagePreview').attr('src', "{{ url('/') }}/" + existingImage).show();
  }
  ...
  $('#staffImage').on('change', function () {
    ...
    if (file) {
      ...
      reader.onload = function (e) {
        $('#staffImagePreview').attr('src', e.target.result).show();
      };
      reader.readAsDataURL(file);
    } else if (existingImage) {
      $('#staffImagePreview').attr('src', "{{ url('/') }}/" + existingImage).show();
    } else {
      $('#staffImagePreview').attr('src', '').hide();
    }
  });

  $('#catchingStaffForm').on('submit', function (e) {
    e.preventDefault();

    var staffId = $('#staffId').val();
    var isEdit = staffId !== '';
    var url = isEdit ? "{{ url('manage-catching-staff') }}/" + staffId : "{{ route('manage-catching-staff.store') }}";
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success && response.hospital) {
          ...
          $('#staffHospital').append(option).val(response.hospital.id);
      ...
      error: function (xhr) {
        ...
        showToast('error', messages.join('\n') || (xhr.responseJSON && xhr.responseJSON.message) || 'Unable to add hospital.');
      }
    });
  }
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success && response.city) {
          var staffCityOption = new Option(response.city.city_name, response.city.id, true, true);
          $('#staffCity').append(staffCityOption).val(response.city.id);

          var hospitalCityOption = new Option(response.city.city_name, response.city.id, true, true);
          $('#hospitalCityInput').append(hospitalCityOption);
      ...
      error: function (xhr) {
        ...
        showToast('error', messages.join('\n') || (xhr.responseJSON && xhr.responseJSON.message) || 'Unable to add city.');
      }
    });
  }
  ...
  $('#hospitalModal').on('hidden.bs.modal', function () {
    $('#hospitalModalForm')[0].reset();
  });
