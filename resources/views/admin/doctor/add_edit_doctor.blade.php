                ...
                <div class="project-top-bar d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                  <div>
                    <h2 class="page-title d-flex align-items-center gap-2">
                      <i class="fa-solid {{ $doctor->exists ? 'fa-user-pen' : 'fa-user-plus' }}"></i>
                    ...
                    <nav aria-label="breadcrumb">
                      <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">
                          <a href="{{ $doctor->exists ? route('manage-doctor.edit', $doctor->id) : route('manage-doctor.create') }}">{{ $doctor->exists ? 'Edit Doctor' : 'Add Doctor' }}</a>
                        </li>
                      </ol>
                    </nav>
                  </div>
                ...
                <div class="card project-list-card" id="project-information-card">
                  ...
                  <div class="card-body">
                    <form id="doctorForm" method="POST" enctype="multipart/form-data">
                      ...
                      <div class="row g-4">
                        <div class="col-md-6">
                          <label class="form-label" for="doctorHospital">Hospital <span class="text-danger">*</span></label>
                          <div class="input-group project-field-group">
                            <select id="doctorHospital" name="hospital_id" class="form-select">
                              <option value="" disabled @selected(! old('hospital_id', $doctor->hospital_id))>Select hospital</option>
                        ...
                        <div class="col-md-6">
                          <label class="form-label" for="doctorName">Name <span class="text-danger">*</span></label>
                          <input type="text" id="doctorName" name="name" class="form-control" value="{{ old('name', $doctor->name) }}" placeholder="Enter doctor name" />
                        </div>

                        <div class="col-md-6">
                          <label class="form-label" for="doctorContact">Contact <span class="text-danger">*</span></label>
                          <input type="tel" id="doctorContact" name="contact" class="form-control" value="{{ old('contact', $doctor->contact) }}" placeholder="Enter contact number" />
                        </div>

                        <div class="col-md-6">
                          <label class="form-label" for="doctorEmail">Email</label>
                          <input type="email" id="doctorEmail" name="email" class="form-control" value="{{ old('email', $doctor->email) }}" placeholder="Enter email address" />
                        </div>

                        <div class="col-md-6">
                          <label class="form-label" for="doctorCity">City <span class="text-danger">*</span></label>
                          <div class="input-group project-field-group">
                            <select id="doctorCity" name="city_id" class="form-select">
                              <option value="" disabled @selected(! old('city_id', $doctor->city_id))>Select city</option>
                              @foreach($cities as $city)
                                <option value="{{ $city->id }}" @selected(old('city_id', $doctor->city_id) == $city->id)>{{ $city->city_name }}</option>
                              @endforeach
                            </select>
                        ...
                        <div class="col-md-12">
                          ...
                          <textarea id="doctorAddress" name="address" class="form-control" rows="3" placeholder="Enter address">{{ old('address', $doctor->address) }}</textarea>
                        </div>

                        <div class="col-md-12">
                          <label class="form-label" for="doctorImage">Image</label>
                          <input type="file" id="doctorImage" name="image" class="form-control" accept="image/*" />
                          <img id="doctorImagePreview" class="doctor-image-preview" src="" alt="Preview" style="display:none;" />
                        </div>
                      </div>

                      <div class="d-flex flex-wrap gap-2 mt-4 form-action-row">
                        <button type="submit" class="btn bg-black text-white">{{ $doctor->exists ? 'Update Doctor' : 'Add Doctor' }}</button>
                        <a href="{{ route('manage-doctor.index') }}" class="btn btn-label-secondary">Cancel</a>
                      </div>
                    </form>
                  </div>
                </div>
...
$(function () {
  ...
  var existingImage = "{{ $doctor->exists ? ($doctor->image ?? '') : '' }}";
  if (existingImage) {
    $('#doctorImagePreview').attr('src', "{{ url('/') }}/" + existingImage).show();
  }

  $('#doctorImage').on('change', function () {
    var file = this.files[0];
    if (file) {
      var reader = new FileReader();
      reader.onload = function (e) {
        $('#doctorImagePreview').attr('src', e.target.result).show();
      };
      reader.readAsDataURL(file);
    } else if (existingImage) {
      $('#doctorImagePreview').attr('src', "{{ url('/') }}/" + existingImage).show();
    } else {
      $('#doctorImagePreview').attr('src', '').hide();
    }
  });
  ...
  $('#doctorForm').on('submit', function (e) {
    ...
    var isEdit = doctorId !== '';
    var url = isEdit ? "{{ url('manage-doctor') }}/" + doctorId : "{{ route('manage-doctor.store') }}";
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success && response.hospital) {
          ...
          $('#doctorHospital').append(option).val(response.hospital.id);
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success && response.city) {
          var doctorCityOption = new Option(response.city.city_name, response.city.id, true, true);
          $('#doctorCity').append(doctorCityOption).val(response.city.id);
