...
@push('styles')
    <style>
      ...
      #manage-vehicle-table {
        margin-bottom: 0;
      }

      #manage-vehicle-table thead th {
        font-size: 0.84rem;
        font-weight: 700;
      ...
      #manage-vehicle-table tbody tr:hover {
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <div class="card manage-project-head-card">
                <div class="project-top-bar d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                  <div>
                    <h2 class="page-title d-flex align-items-center gap-2"><i class="fa-solid fa-truck-medical"></i> Manage Vehicle</h2>
                  ...
                  @can('add vehicle')
                  <a href="{{ route('manage-vehicle.create') }}" class="addbtn btn btn-dark">
                ...
                <div class="card project-list-card">
                  ...
                  <div class="card-body">
                    <div class="table-responsive">
                      <table id="manage-vehicle-table" class="table table-striped border-top">
                        <thead>
                          <tr>
                            <th>ID</th>
                            <th>Vehicle</th>
...
$(function () {
  ...
  var table = $('#manage-vehicle-table').DataTable({
    processing: true,
    serverSide: true,
    ajax: "{{ route('manage-vehicle.index') }}",
    columns: [
      { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
      { data: 'vehicle_number', name: 'vehicle_number' },
      { data: 'city_name', name: 'city.city_name' },
      { data: 'login_id', name: 'login_id' },
      { data: 'password_mask', name: 'password_mask', orderable: false, searchable: false },
  ...
  $(document).on('click', '.item-delete', function () {
    ...
    confirmDelete(function () {
      $.ajax({
        url: "{{ url('manage-vehicle') }}/" + vehicleId,
        method: 'POST',
        data: { _method: 'DELETE' },
        ...
        error: function (xhr) {
          showToast('error', (xhr.responseJSON && xhr.responseJSON.message) || 'Unable to delete vehicle.');
        }
      });
    });
  });
  ...
  $(document).on('change', '.toggle-status', function () {
    ...
    $.ajax({
      url: "{{ url('manage-vehicle') }}/" + vehicleId + '/toggle-status',
      method: 'POST',
      success: function (response) {
        showToast('success', response.message);
        table.ajax.reload(null, false);
